use std::{fs::File, path::Path, io::{BufRead, BufReader, Write}};
...
fn main() {
    ...
    if header_data.total != current_devices.len(){
        ...
        for count in 0..current_devices.len(){
            ...
            if current_keyboard.port_number != saved_keyboard.port_number {
                ...
                if REPORT_ON_PORT_CHANGE == true {
                    device_passed[count] = false;
                }
            }

            if device_passed[count] == false {
                all_passed = false;
            }
    ...
    if all_passed == false || REPORT_ON_PASS == true {
...
fn load_hid() -> (Vec<Keyboard>, Header) {
    ...
    return (saved_devices, header_data);
